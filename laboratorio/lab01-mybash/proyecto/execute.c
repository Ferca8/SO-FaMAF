...
// ESTO VA AL FINAL SÍ O SÍ O SE ROMPE
...
static void execute_two_commands(pipeline apipe) {
  ...
  if (pid1 == 0) {              // PRIMER HIJO
    ...
    execute_external_command(
        pipeline_front(apipe)); // Ejecuta el primer comando
  }
  ...
  if (pid2 == 0) {             // SEGUNDO HIJO
    ...
    close(fd[1]);              // Cierra la punta de escritura del pipe
    execute_external_command(
        pipeline_front(apipe)); // Ejecuta el segundo comando
  } else {                      // PADRE
    close(fd[0]);               // Cierra la punta de lectura del pipe
    close(fd[1]);               // Cierra la punta de escritura del pipe
...
static void execute_n_commands(pipeline apipe) {
  ...
  int *pid_table = malloc(
      pl_length * sizeof(int)); // Tabla para guardar los PIDs de los hijos

  for (int i = 0; i < pl_length; i++) {
    if (i > 0) {                      // Si no es el primer proceso...
      previous_fd[0] = current_fd[0]; // Guardo los file descriptors del proceso
                                      // anterior para luego
    ...
    if (pid < 0) {
      ...
      if (i > 0) {                          // Si no es el primer proceso...
        dup2(previous_fd[0], STDIN_FILENO); // Conecto el stdin a la punta de
                                            // lectura del pipe anterior
        close(previous_fd[0]);
        close(previous_fd[1]);
      }
      if (i < pl_length - 1) {              // Si no es el último proceso...
        dup2(current_fd[1], STDOUT_FILENO); // Conecto el stdout a la punta de
      ...
      pid_table[i] = pid; // Guardo el PID para que el proceso padre pueda
                          // esperar a su hijo
...
void execute_pipeline(pipeline apipe) {
  ...
  if (pipeline_is_empty(
          apipe)) { // Si el pipeline está vacío, no hay nada que ejecutar
    printf("La pipeline está vacía.\n");
    return;
  } else if (builtin_alone(apipe)) { // Si el pipeline tiene un solo builtin, lo
    ...
    } else if (pipeline_length(apipe) ==
               2) { // Si hay dos comandos, los ejecuta conectados con una pipe
      execute_two_commands(apipe);
    } else { // Si hay n comandos (> 2), los ejecuta conectando una pipe entre
             // cada uno
