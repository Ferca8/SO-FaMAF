...
/*
    Para FUN. Printea ronda actual, hace un pequeño delay y borra la terminal 
    para pasar al siguiente frame.
*/
void
...
/*
    Si sem_ping es positivo imprime "ping", luego libera al semáforo sem_pong 
    para que continúe con el siguiente proceso.
*/
void
...
/*
    Si sem_pong es positivo imprime "pong", luego libera al semáforo sem_ping 
    para que continúe con el siguiente proceso.
*/
void
...
/*
    La idea general de pingpong consiste en la utilización de dos semáforos: uno 
    para controlar el proceso que imprime "ping" y otro para controlar el proceso 
    que imprime "pong".
    Al inicializarlos uno toma el valor de 1 y el otro de 0, haciendo que luego 
    mediante las syscalls sem_down y sem_up se vaya alternando la ejecución de 
    ambos procesos de manera coordinada.
*/
int
main(int argc, char *argv[])
{
    ...
    if (rc == 0) {    // Proceso hijo, que imprime "ping".
      ...
      sem_down(sem_ping);  // Ping() es el que primero termina, debe esperar a 
                           // la ultima señal que genera pong() para retornar.
    }
